...
document.addEventListener('DOMContentLoaded', function() {
  const canvas = document.getElementById('ballTrajBBoxCanvas');
  const canvasBirdEye = document.getElementById('ballTrajBBoxBirdEyeCanvas');
  const ctx = canvas.getContext('2d');
  const ctxBirdEye = canvasBirdEye.getContext('2d');
  const tSlider = document.getElementById('ballTrajBBoxTSlider');
  ...
  tSlider.addEventListener('input', function() {
    ...
    document.getElementById('ballTrajBBoxTValue').textContent = t.toFixed(1);
    drawVisualization();
  });
  ...
  function drawCurrentBall(canvas, homographyMatrix, centeredMatrix, force_null_z=false, drawBbox=false) {
    if (!posT) return;
    const posCurrentT = [...posT[t]];
    if (force_null_z) {
      posCurrentT[2] = 0.0;
    }
    drawBallProjection(canvas, posCurrentT, BasketballBallRadius, homographyMatrix, centeredMatrix, 'rgba(226, 130, 40, 1)', 'rgba(7, 7, 7, 1)', drawBbox, 'blue');
  }
  ...
  function drawVisualization() {
    ...
    drawCurrentBall(canvas, BasketballTrajHomographyMatrix, BasketballTrajHomographyCenteredMatrix, false, true);
