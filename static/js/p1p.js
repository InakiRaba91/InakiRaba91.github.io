...
import { transform4DOFObject } from './utils/transformDOF.js';

document.addEventListener('DOMContentLoaded', function() {
  const tSlider = document.getElementById('t-p1p-slider');
  const rxSlider = document.getElementById('rx-p1p-slider');
  const rySlider = document.getElementById('ry-p1p-slider');
  const rzSlider = document.getElementById('rz-p1p-slider');
  const canvasCamera = document.getElementById('interactive-plot-p1p-camera-view');
  const canvasCameraBg = document.getElementById('interactive-plot-p1p-camera-view-bg');
  const canvasFrame = document.getElementById('interactive-plot-p1p-frame-view');
  const canvasFrameBg = document.getElementById('interactive-plot-p1p-frame-view-bg');
  ...
  function updateCanvas() {
    ...
    const paramsDOF = {t: t, rx: rx, ry: ry, rz: rz};

    // Update the displayed values
    document.getElementById('t-p1p-value').textContent = t;
    document.getElementById('rx-p1p-value').textContent = rx;
    document.getElementById('ry-p1p-value').textContent = ry;
    document.getElementById('rz-p1p-value').textContent = rz;
    
    // Draw the camera view
    const vectorRefToPinhole = displayCameraView(ctxCamera, canvasCamera, LengthsObj, paramsDOF, transform4DOFObject, indicesPtsRef)
    displayFrameView(ctxFrame, canvasFrame, vectorRefToPinhole, LengthsObj, paramsDOF, transform4DOFObject, indicesPtsRef)
  }
