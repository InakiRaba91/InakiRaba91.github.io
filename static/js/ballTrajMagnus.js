...
document.addEventListener('DOMContentLoaded', function() {
  const canvas = document.getElementById('ballTrajMagnusCanvas');
  const canvasBirdEye = document.getElementById('ballTrajMagnusBirdEyeCanvas');
  const ctx = canvas.getContext('2d');
  const ctxBirdEye = canvasBirdEye.getContext('2d');
  const omegaSlider = document.getElementById('ballTrajMagnusOmegaSlider');
  const tSlider = document.getElementById('ballTrajMagnusTSlider');
  let omegaZ = parseFloat(omegaSlider.value);
  let x = -54.0;
  let y = -8.0;
  let z = 0.0;
  let vMag = 80.0;
  const unit_vel = [0.85955781, -0.12494379, 0.49552943].map(val => parseFloat(val));
  ...
  omegaSlider.addEventListener('input', function() {
    ...
    document.getElementById('ballTrajMagnusOmegaValue').textContent = omegaZ.toFixed(1);
    computeAndDrawTrajectory();
  });

  tSlider.addEventListener('input', function() {
    t = parseFloat(this.value);
    document.getElementById('ballTrajMagnusTValue').textContent = t.toFixed(1);
    drawVisualization();
  });
  ...
  function computeAndDrawTrajectory() {
    ...
    let pos0 = [x, y, z].map(val => parseFloat(val));
