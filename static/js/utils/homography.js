import { multiplyMatrix, inverseMatrix } from './matrix.js';
...
export function backProjectPt(ptImgCanvas, homography, PitchDims, imgCanvasSize, pitchCanvasSize, origFrameSize) {
  ...
  let [x, y, z] = multiplyMatrix(homography_inv, [ptImg.x, ptImg.y, 1]);
  const xNorm = x / z;
  const yNorm = y / z;
...
export function projectPt(ptImgPitchCanvas, homography, PitchDims, imgCanvasSize, pitchCanvasSize, origFrameSize) {
  ...
  let [x, y, z] = multiplyMatrix(homography, [xPitch, yPitch, 1]);
  const xFrame = x / z;
  const yFrame = y / z;
